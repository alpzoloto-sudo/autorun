                         ...
                         int cursor_x, int cursor_y, int cursor_visible )
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        draw_quad( comp, quad->texture, quad->x, quad->y, quad->width, quad->height,
                   quad->src_x, quad->src_y,
                   quad->src_width ? quad->src_width : quad->width,
                   quad->src_height ? quad->src_height : quad->height );
    }
    ...
    if (cursor_visible && comp->cursor.name)
    {
        ...
        draw_quad( comp, &comp->cursor, cursor_x, cursor_y, POINTER_CURSOR_W, POINTER_CURSOR_H,
                   0, 0, POINTER_CURSOR_W, POINTER_CURSOR_H );
        gl->Disable( GL_BLEND );
    }
}
